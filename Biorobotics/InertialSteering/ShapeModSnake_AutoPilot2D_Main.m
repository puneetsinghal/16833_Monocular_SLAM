...
% Parameters for Stuck/Unstuck Analysis
filterLen = round(2 * fbkHz * 2 * pi / (100 * snakeConst.tmFreq)) * 100;
freqencies = fbkHz * (0 : filterLen/2)/filterLen;
fftCount = 1;
accelMatrix = zeros(filterLen, snakeConst.numModules);
mF = zeros(filterLen, 1);
dtau = 1/(dt * filterLen);
peakFreq = snakeConst.tmFreq /(2 * pi);

% Boolean value that reflects whether the snake is 
stuck = 0;
cooldownS = 0;
cooldownU = 0;
cooldownThres = 1000;
...
%     accelMatrix(fftCount, :) = fbk.torque(1:numModules);
    accelMatrix(fftCount, 2:2:numModules) = fbk.accelY(:,1:2:numModules);
    accelMatrix(fftCount, 1:2:numModules) = fbk.accelX(:,2:2:numModules);
    if(t > filterLen*dt)
        maxFreq = get_maxFreq(filterLen, accelMatrix, dt);
        mF(mod(fftCount, filterLen) + 1, :) = maxFreq;
        
        mmF = mean(mF(mF ~= 0));
       
        if abs(mmF - peakFreq) < dtau
            stuck = 1;
            disp('STUCK!!!!');
        end
    end
    
    fftCount = mod(fftCount, filterLen) + 1;
    
    if stuck
        % New Heading = current heading + pi/2 in the direction of the
        % target heading.
        
%         targetHeading
        headingOffset
        if cooldownS > cooldownThres
            if headingOffset ~= targetHeading
%                 disp('turning back')
                headingOffset = targetHeading;
            else
%                 disp('Turning away');
                headingOffset = min(mod(headingOffset + pi/2, 2 * pi) - pi, mod(headingOffset - pi/2, 2 * pi) - pi);
            end
            cooldownS = 0;
        end
        ...
        if (headingOffset ~= targetHeading)
            if (cooldownU > cooldownThres/20)
%                 disp('turning back')
                
                headingOffset = targetHeading;
            end
            disp('ustuck')
            cooldownU = cooldownU + 1;
        end
        cooldownS = 0;
    end
    ...
    tau_Applied            = get_AppliedForce2D(tau_Ext, ...
                                          snakeConst, snakeCompl, windows);
    [snakeCompl, snakeNom] = get_NewNomParam2D(snakeCompl, snakeNom, ...
    ...
    if isnan(mainAxis0)
        [~,mainAxis0]      = get_NewAngles2D(snakeCompl, snakeConst, windows);
    end
...
if saveQ
	...
	save(FileName, 'steeringValues', 'headingValues', 'headingOffsetValues', 'shapeRegularity', 'currSerpPos0', 'mainAxis0', 'headingOffsets', 'snakeConst', 'snakeCompl', 'snakeNom', 'windows', 't', 'dt');
end
