...
%         end
        ...
        targetHeading = snakeNom.h;
%         display(snakeNom.h);
    else
        snakeNom.h = 0;
    end
    
%     accelMatrix(mod(fftCount, filterLen) + 1, 1:2:numModules)   = fbk.accelX(:,1:2:numModules);
%     accelMatrix(mod(fftCount, filterLen) + 1, 2:2:numModules)   = fbk.accelY(:,2:2:numModules);
%     if(fftCount > filterLen)
%         disp('Here!!!');
%         mmF = mean(mF);
%         if (abs(mmF - peakFreq) < 2*dtau)
%             stuck = 1;
%             disp('STUCK!!!!');
%         end
%     end
...
%     fftCount = fftCount + 1;
%     
%     % Calls the function that determines whether the snake is stuck
%     if stuck
%         % New Heading = current heading + pi/2 in the direction of the
%         % target heading.
%         snakeNom.h = min(mod(snakeNom.h + pi/2, 2 * pi) - pi, mod(snakeNom.h - pi/2, 2 * pi) - pi);
%         disp(min(mod(snakeNom.h + pi/2, 2 * pi) - pi, mod(snakeNom.h - pi/2, 2 * pi) - pi));
%     end
%     
%     if snakeNom.h ~= targetHeading
%         if cooldown > cooldownThres 
%             cooldown = 0;
%             disp('Turning back');
%             snakeNom.h = targetHeading;
%         end
%         cooldown = cooldown + 1;
%     end
%     
